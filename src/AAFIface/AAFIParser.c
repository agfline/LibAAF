...
static int parse_TimelineMobSlot( AAF_Iface *aafi, aafObject *TimelineMobSlot, td *__ptd );
static int parse_EventMobSlot( AAF_Iface *aafi, aafObject *EventMobSlot, td *__ptd );
...
static int parse_DescriptiveMarker( AAF_Iface *aafi, aafObject *DescriptiveMarker, td *__ptd );
...
static int retrieve_UserComments( AAF_Iface *aafi, aafObject *UserComments, aafiMetaData **metadataList );
...
static int parse_Transition( AAF_Iface *aafi, aafObject *Transition, td *__ptd )
{
	...
	if ( fadeType & AAFI_TRANS_FADE_OUT ) {
...
static int parse_OperationGroup( AAF_Iface *aafi, aafObject *OpGroup, td *__ptd )
{
	...
	if ( aafUIDCmp( OperationIdentification, &AAFOperationDef_MonoAudioDissolve ) ) {
		...
		if ( ComponentAttributeList ) {
			...
			if ( curveType ) {
				switch ( *curveType ) {
					case AVID_MEDIA_COMPOSER_CURVE_TYPE_LINEAR:      Trans->flags &= ~(AAFI_INTERPOL_MASK); Trans->flags |= AAFI_INTERPOL_LINEAR; break;
					case AVID_MEDIA_COMPOSER_CURVE_TYPE_EQUAL_POWER: Trans->flags &= ~(AAFI_INTERPOL_MASK); Trans->flags |= AAFI_INTERPOL_POWER;  break;
					default: debug( "Unknown Avid Media Composer fade curve: %i", *curveType ); break;
				}
			}
		}
		...
		if      ( aafi->ctx.current_combined_clip_total_channel == 2 ) { track_format = AAFI_TRACK_FORMAT_STEREO; }
		else if ( aafi->ctx.current_combined_clip_total_channel == 6 ) { track_format = AAFI_TRACK_FORMAT_5_1;    }
		else if ( aafi->ctx.current_combined_clip_total_channel == 8 ) { track_format = AAFI_TRACK_FORMAT_7_1;    }
		else {
		...
		if ( !Parameters ) {
			...
			rc = -1; goto end; /* we still have to parse InputSegments, so we're not losing any clip */
		}
...
void aafi_dump_obj( AAF_Iface *aafi, aafObject *Obj, struct trace_dump *__td, int state, const char *func, int line, const char *fmt, ... )
{
	va_list args;
	...
	if ( aafi->ctx.options.trace == 0 ) {
		...
		switch ( state ) {
			case TD_ERROR:         verbtype = VERB_ERROR;   break;
			case TD_WARNING:
			case TD_NOT_SUPPORTED: verbtype = VERB_WARNING; break;
			// case TD_INFO:
	...
	if ( Obj ) {
		...
		switch ( state ) {
			case TD_ERROR:		      LOG_BUFFER_WRITE( log, "%serr %s%s %s", ANSI_COLOR_RED(log),    ANSI_COLOR_DARKGREY(log), TREE_LINE, ANSI_COLOR_RED(log)    );  break;
			case TD_WARNING:	      LOG_BUFFER_WRITE( log, "%swrn %s%s %s", ANSI_COLOR_YELLOW(log), ANSI_COLOR_DARKGREY(log), TREE_LINE, ANSI_COLOR_YELLOW(log) );  break;
			case TD_NOT_SUPPORTED:  LOG_BUFFER_WRITE( log, "%suns %s%s %s", ANSI_COLOR_ORANGE(log), ANSI_COLOR_DARKGREY(log), TREE_LINE, ANSI_COLOR_ORANGE(log) );  break;
			default:                LOG_BUFFER_WRITE( log, "    %s%s ",     ANSI_COLOR_DARKGREY(log), TREE_LINE );  break;
		}
	...
	if ( Obj ) {
		...
		switch ( state ) {
			case TD_ERROR:          LOG_BUFFER_WRITE( log, "%s", ANSI_COLOR_RED(log)      );  break;
			case TD_WARNING:        LOG_BUFFER_WRITE( log, "%s", ANSI_COLOR_YELLOW(log)   );  break;
			case TD_NOT_SUPPORTED:  LOG_BUFFER_WRITE( log, "%s", ANSI_COLOR_ORANGE(log)   );  break;
			case TD_INFO:
			case TD_OK:             LOG_BUFFER_WRITE( log, "%s", ANSI_COLOR_CYAN(log)     );  break;
		}
