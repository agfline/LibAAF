...
static int utf16CodeToUTF8( const uint16_t *u16Code, char *u8Code, int *u16Len, int *u8Len );
...
int laaf_util_snprintf_realloc( char **str, size_t *size, size_t offset, const char *format, ... )
{
	size_t tmpsize = 0;
...
FILE * laaf_util_fopen_utf8( const char *filepath, const char *mode )
{
	FILE *fp = NULL;
...
int laaf_util_vsnprintf_realloc( char **str, size_t *size, size_t offset, const char *fmt, va_list args )
{
	FILE *dummy = NULL;
	va_list args1;
...
int laaf_util_dump_hex( const unsigned char *stream, size_t stream_sz, char **buf, size_t *bufsz, size_t offset, const char *padding )
{
	if ( stream == NULL ) {
		return -1;
	}
	...
	while ( count < stream_sz ) {
		...
		for ( i=0; i < lineLen; i++ ) {
			...
			if ( i < 8 ) {
				if ( isalnum( *(stream+count+i) ) ) ascii[i] = *(const char*)(stream+count+i);
				else                                ascii[i] = '.';
			}
			else
			if ( i > 8 ) {
				if ( isalnum( *(stream+count+i) ) ) ascii[i+1] = *(const char*)(stream+count+i);
				else                                ascii[i+1] = '.';
			}
			else {
