...
void cfb_dump_node( CFB_Data *cfbd, cfbNode *node, int print_stream, const char *padding )
{
	if ( node == NULL )
		return;
...
void cfb_dump_nodePath( CFB_Data *cfbd, const char *path, int print_stream, const char *padding )
{
...
void cfb_dump_nodePathStream( CFB_Data *cfbd, const char *path, const char *padding )
{
	struct aafLog *log = cfbd->log;
...
void cfb_dump_nodePaths( CFB_Data *cfbd, uint32_t prevPath, char *strArray[], uint32_t *str_i, cfbNode *node, const char *padding, int firstIteration )
{
	struct aafLog *log = cfbd->log;
